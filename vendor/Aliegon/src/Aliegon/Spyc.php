...
public function loadLanguageFile ($file) {
    $time = filemtime($file);
    $configfile = ROOT . '/cache/language_' . $time . '.php';
    if(file_exists($configfile)){
      include_once $configfile;
      return $language;
    }
     if (is_writable(ROOT . '/cache/'))
      if ($files = glob(ROOT . '/cache/language_*.php'))
        foreach ($files as $delfile)
          unlink($delfile);

    $parsed = $this->__load($file);
    if(is_writable(ROOT . '/cache/')){
      $fp = fopen ( $configfile, 'w' );
      fwrite( $fp, $parsed_string = "<?php" . "\n" . "\$language = " . var_export( $parsed, TRUE ) . ";" . "\n?>", strlen( $parsed_string ) );
      fclose( $fp );
    }
    return $parsed;
  }

public function loadRoutingFile ($file) {
    $time = filemtime($file);
    $configfile = ROOT . '/cache/routing_' . $time . '.php';
    if(file_exists($configfile)){
      include_once $configfile;
      return $routing;
    }
     if (is_writable(ROOT . '/cache/'))
      if ($files = glob(ROOT . '/cache/routing_*.php'))
        foreach ($files as $delfile)
          unlink($delfile);
    ...
    if(is_writable(ROOT . '/cache/')){
      $fp = fopen ( $configfile, 'w' );
      fwrite( $fp, $parsed_string = "<?php" . "\n" . "\$routing = " . var_export( $parsed, TRUE ) . ";" . "\n?>", strlen( $parsed_string ) );
      fclose( $fp );
    }
    return $parsed;
  }
  ...
  public function loadConfigFile ($file) {
    $time = filemtime($file);
    $configfile = ROOT . '/cache/config_' . $time . '.php';
    if(file_exists($configfile)){
      include_once $configfile;
      return $config;
    }
     if (is_writable(ROOT . '/cache/'))
      if ($files = glob(ROOT . '/cache/config_*.php'))
        foreach ($files as $delfile)
          unlink($delfile);
